...
However separate lines is not much comfortable to work with. It is much more convenient to manipulate the whole record as a single object. The first step to «squash» lines into the records is concatenating multi-lined data in GEDCOM that marked with `CONT` and `CONC` tags:
